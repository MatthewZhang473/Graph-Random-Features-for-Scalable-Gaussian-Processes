...
def load_cora(num_train, num_test):
    ...
    node_data = pd.read_csv(
        os.path.join(data_dir, "cora.content"), sep="\t", header=None, index_col=0
    )
    # Our GP model doesn't use bag of words
    node_subject = pd.DataFrame(node_data.iloc[:, -1]).rename(
        {1434: ""}, axis="columns"
    )
    ...
    train_id, test_id = (
        random_permutation[:num_train],
        random_permutation[num_train : num_train + num_test],
    )
...
def load_PEMS(num_train=250, dtype=np.float64, path="data"):
    # unzip daata
    with zipfile.ZipFile(os.path.join(path, "PEMS.zip"), "r") as zip_ref:
        zip_ref.extractall(path)
    # Data reading
    with open(os.path.join(path, "PEMS/adj_mx_bay.pkl"), "rb") as f:
        sensor_ids, sensor_id_to_ind, _ = pickle.load(f, encoding="latin1")
    all_signals = pd.read_hdf(os.path.join(path, "PEMS/pems-bay.h5"))
    coords = pd.read_csv(
        os.path.join(path, "PEMS/graph_sensor_locations_bay.csv"), header=None
    )

    # Loading real world graph of roads
    north, south, east, west = 37.450, 37.210, -121.80, -122.10
    if not os.path.isfile(os.path.join(path, "PEMS/bay_graph.pkl")):
        ...
        with open(
            os.path.join(path, "PEMS/bay_graph.pkl"), "wb"
        ) as f:  # frequent loading of maps leads to a temporal ban
            pickle.dump(G, f)
    else:
        with open(os.path.join(path, "PEMS/bay_graph.pkl"), "rb") as f:
            G = pickle.load(f)
